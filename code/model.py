# code reference: https://github.com/AntixK/PyTorch-VAE
...
class VAE(nn.Module):
            ...
            for h_dim in hidden_dims:
                modules.append(
                    nn.Sequential(
                        ...
                        nn.LeakyReLU())  # the original paper use tf.nn.softplus
                )
                current_in = h_dim
    ...
    def encode(self, x: List):
        ...
        for x_i, encoder, fc_mu, fc_var in zip(x, self.encoder, self.fc_mu, self.fc_var):
            if x_i is not None:
                ...
                mu += [qz_m]  # TODO: check if correct, this is a placeholder
                log_var += [qz_logv]  # TODO: won't be used b.c. elbo_combn
        mu = torch.cat(mu, dim=0)
        log_var = torch.cat(log_var, dim=0)
        return mu, log_var

    def forward(self, x: List, elbo_combn=None, kl_penalty=0):
        """
        Current implementation of kl_penalty only works for 2 omics
        """
        x, dataset = x
        n_modality = len(x)
        ...
        for elbo_bool in elbo_combn:
            ...
            z = self.reparameterize(mu_subset, var)  # z shape: 128x10
            ...
            for beta_i, x_i, beta_dt_i, beta_dt_mtp_i, dt_i, n_f in \
                ...
                # dt_i: 128, fac_to_mat(dt_i, n_f): 128x1, beta_dt_i: 1xn_feature, res: 128xn_features
                ...
                recon_loss += - torch.sum(  # the results before sum should equal to the batch size
                    x_i * torch.log(torch.softmax(
                        # beta_i shape: 10xn_feature, res shape: 128xn_features
            ...
            if elbo_bool == [True, True] and kl_penalty != 0:
                # scale parameter is the standard deviation of the distribution
                # KL is additive for independent dist.
                p_mu, p_var = self.experts(mu[[True, True, False]], log_var[[True, True, False]])
                q_mu, q_var = self.experts(mu[[True, False, True]], log_var[[True, False, True]])
                p = torch.distributions.Normal(p_mu, p_var.sqrt())
                q = torch.distributions.Normal(q_mu, q_var.sqrt())
                kl_loss = torch.distributions.kl_divergence(p, q) + torch.distributions.kl_divergence(q, p)
                latent_loss += kl_penalty * kl_loss.sum()
        return latent_loss, recon_loss

    def latent_loss(self, mu0, var0, mu1, var1):  # note here mu0/var0 and mu1/var1 have different dimensions
        latent_loss = 0.5 * torch.sum(  # pytorch minimize risk, but we want maximize ELBO
    ...
    @torch.no_grad()
    def get_marginal_likelihood(self, x, elbo_bool=None, rep=100): # TODO check
        if elbo_bool is None:
            elbo_bool = [True]*len(x)
        ...
        l_sum = 0
        for i in range(rep):
            z = self.reparameterize(self.mu2, self.var2)
            recon_loss = 0
            for beta_i, x_i in zip(beta_subset, x_subset):
                recon_loss += - torch.sum(  # the results before sum should equal to the batch size
                    x_i * torch.log(torch.softmax(
                        torch.matmul(torch.softmax(z, dim=1), beta_i),
                        dim=1)))
            l_sum += recon_loss.cpu().numpy()
        return l_sum / rep

    @torch.no_grad()
    def get_marginal_likelihood_1(self, x, elbo_bool=None, rep=100): # TODO check
        """
        Reference scVI
        """
        if elbo_bool is None:
