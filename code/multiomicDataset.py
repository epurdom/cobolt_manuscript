...
class MultiomicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dat = [self.dt[om]['counts'][self.barcode_in_om[om][b]]
               if self.barcode_in_om[om][b] is not None else None
               for om in self.omic]
        dataset = [self.dt[om]['dataset'][self.barcode_in_om[om][b]]
                   if self.barcode_in_om[om][b] is not None else None
                   for om in self.omic]
        return dat, dataset
    ...
    def get_comb_idx(self, omic_combn):
        bl = [self.dt[om]['barcode']
              for om, include in zip(self.omic, omic_combn) if include]
        b = bl[0]
        for x in bl[1:]:
...
def collate_wrapper(batch, omic_combn): # TODO
    # check if misssing modalities are consistent
    ...
    batch = [torch.from_numpy(sparse.vstack(x).toarray()).float()
